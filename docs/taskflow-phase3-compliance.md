...
[
  {
    ...
    "prior_turn_depth": 1,
    "taskflow_board_id": "board-seci-taskflow",
...
- `taskflow_board_id`: optional explicit board id for replay-only container
  env injection. Use this when a production Taskflow DB snapshot has a
  historical `boards.group_folder` that differs from the local fixture folder.
...
Production WhatsApp corpora generated by `scripts/prod-whatsapp-corpus.ts`
include `taskflow_board_id` when the sibling Taskflow snapshot can resolve the
board by group JID. This keeps replay fixture folder aliases from breaking
Taskflow board env injection when historical production folder names changed.
