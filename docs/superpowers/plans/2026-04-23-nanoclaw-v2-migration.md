...
- [ ] Step 4b: author `engage_pattern` configuration in board provisioning seed scripts. **Set `engage_mode='pattern'` + `engage_pattern='.'`** (always engage) per migrate-v2 driver convention (`/root/nanoclaw-migrate-v2/setup/migrate/seed-v2.ts:375-386`). Setting `@<trigger>` would DROP messages without the literal mention — production regression. Per-board agent prefix (`Case:` / `Audit:`) is an **agent-side convention** in CLAUDE.md prompts, NOT v2-router-driven dispatch.
- [ ] Step 4c: author **board_admins → user_roles seeder**. The migrate-v2 driver only seeds the global owner + members for v1's allowlist; it does NOT migrate TaskFlow's `board_admins` table to scoped `user_roles` rows. Per memory `project_v2_user_roles_invariant.md`: map `board_admins.is_primary_manager=1` to `role='admin'` with `agent_group_id=<board-id>` (NEVER scoped 'owner' — v2 forbids it).
- [ ] Step 4d: populate `~/.config/nanoclaw/mount-allowlist.json` with TaskFlow's required mounts during install. v2's mount-security module is a **default loader** that silently blocks `additionalMounts` if the allowlist file is missing (`src/container-runner.ts:323-327`, `src/modules/mount-security/index.ts:74-83, 230-239`). Without this step, board folders won't mount and TaskFlow boards fail to spawn containers.
- [ ] Step 4e: TaskFlow's cross-board approval STAYS fork-private as `subtask_requests` table. Per Codex review #5: v2's `ask_user_question` has a 300s timeout and FILTERS late `kind='system'` responses (poll-loop.ts:66-67), so it cannot replace multi-day cross-board approval workflows. `ask_user_question` is adopted only for ≤300s in-session prompts. See `docs/superpowers/audits/2026-05-02-v2-features-for-our-skills.md` for full context.
