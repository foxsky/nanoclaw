...
- ⚙️ Task 1.7: **GATE PASSED for build + tests + auditor-day-1 (2026-05-01 EOD+1, Codex review #3 corrected framing).** Phase 1 is NOT end-to-end-workload-closed — that requires Phase 0.5 (operator phone) for real WhatsApp inbound → container → Claude → outbound. Scope of what IS closed:
  - `bunx tsc --noEmit` ✅
  - `bun test` → 719 pass / 45 fail / 6 errors / 1 todo across 13 files. The 46 non-pass results are ALL vitest-specific mocking API gaps (`vi.stubGlobal`, `vi.unstubAllGlobals`) — NOT bun:sqlite port issues. Test framework migration scoped to Phase 6 cleanup. Real bun:sqlite-vs-better-sqlite3 deltas: `.get()` returns `null` (not `undefined`) on empty match — fixed in 5 test sites + 2 heredoc production sites (caught by F9 paired-output diff, see below). Production .ts code uses truthy guards, so unaffected.
  - **Container build SUCCEEDED** via `DOCKER_BUILDKIT=1 ./container/build.sh v2-feat`. Image `nanoclaw-agent:v2-feat` is 5.5GB / 1.71GB content. Bun 1.3.12 + UID 1000 confirmed inside image.
  - **Auditor heredoc smoke PASSED** under bun against prod snapshot. Extracted auditor.js (1445 lines), ran via `bun`. **F9 paired-output diff (2026-05-01 EOD+1):** v1 (node + better-sqlite3) and v2 (bun + bun:sqlite) auditor runs against the SAME prod snapshot now produce **byte-identical JSON output** (md5 match `301eead187f079beed3c1fd2e273f77c`). Plan gate "output matches 1.x within ±5%" is exceeded — output matches 100% **for that one snapshot day**. Codex review #3 (2026-05-01 EOD+1) flagged: paired-diff against ONE day is strong evidence for that day's data shape; covers neither sparse-board nor noisy-attribution edge cases. Phase 4 shadow run on test boards over 5 days is the proper validation gate. Two `.get(...) !== undefined` bugs caught and fixed at `auditor-script.sh:872` (turnMembershipMatch) and `:969` (scheduledTasksStmt) — both inverted booleans under bun:sqlite. See memory `feedback_get_returns_null_in_bun_sqlite.md`.
  - **NOT closed: end-to-end workload validation.** No test exercises the real WhatsApp inbound → container spawn → Claude API call → outbound delivery path. Phase 0.5 (operator test phone) is the gate for that; Phase 4 shadow run is the multi-day validation. Phase 1's "behavior-closed" applies only to auditor-day-1 + unit/integration tests + container build, NOT to real workload.
