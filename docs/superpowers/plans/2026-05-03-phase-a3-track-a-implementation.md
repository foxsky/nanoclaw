...
### Step 2.1 — Create branch + scaffold

```bash
git checkout skill/whatsapp-fixes-v2
...
| 2.3.j Post-provisioning send + ACL refresh (Discovery 14) | engine integration | Per Discovery 14: skill calls `writeDestinations(parent_id, sess_id)` directly for affected agent_groups. `wakeContainer` on running container is no-op. Dense seed ~784 `agent_destinations` rows (28 boards × 27 peers) at cutover. Test: provision new board, immediately send from existing parent → assert delivered. |
| 2.3.k Engage config override (Discovery 11) | apply-engage-config.sql (target `data/v2.db`) | **Override** migrate-v2 driver default: set all 28 `messaging_group_agents` to `engage_mode='pattern' + sender_scope='known' + ignored_message_policy='accumulate' + unknown_sender_policy='request_approval'`. **Open question:** engage_pattern `'.'` (~10k wakes/day) vs `'(@Case\|@Tars\|^/[a-z])'` (~2-3k wakes/day) per Discovery 11 — **needs user call before test passes**. |
