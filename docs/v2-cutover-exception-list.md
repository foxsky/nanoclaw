...
| Item | Finding | Done? |
|------|---------|-------|
| V1 board/user creation audit | Empirical sweep of the cutover corpus (12,737 msgs → 49 traced creation/registration episodes), cross-referenced to `taskflow.db`. **The earlier "v1 creation always worked" claim is RETIRED:** every *affirmed* request landed as at least a person row (0 omissions), but Sanunciel is a partial creation failure and the async child-board path had **6 defects**. Per-defect source verification (2026-05-30): **2 fully prevented going forward** (Hudson dup → `alreadyOnThisParent` guard, Edilson name → division-name guard), **1 boot-repaired** (Jefferson name-heal), **1 host-fixed/container-residual** (Reginaldo → FU-4), **2 that persist** → **EX-014 (Sanunciel, non-atomic skip)** + **EX-015 (Mariany, dual person_id)**. Migration repairs nothing (verbatim copy), so **3 data populations** need a one-time fix: Sanunciel orphan, Mariany dup-id, and the `board-po-setd-secti-taskflow-2` duplicate board (Hudson residual). Full map: `docs/v1-creation-empirical-map.md`. **All 6 fixes shipped + Phase-3-style replay-validated 2026-05-30 (6/6 PASS): each historical defect scenario was replayed through the FIXED code — EX-015 / Jefferson / Edilson against the real engine/boot code on forks of the cutover snapshot; EX-014 / Hudson / FU-4 via the regression harness + real-snapshot precondition checks. Watch items (post-cutover, not blockers): (a) Hudson `alreadyOnThisParent` guard blocks a re-fire once the registry row exists but does not close the true concurrent pre-commit TOCTOU window (not exercised against a live WhatsApp adapter); (b) EX-015's literal historical sender hit the manager-auth gate before reconcile — the reconcile logic is sender-independent so the fix is validated, but confirm with the owner whether a delegate-tier sender should be able to complete a full register.** | ◑ 2 `proposed` exceptions open — must resolve before cutover (Checklist #2) |
...
- **Mitigation / followup:** (a) **Data — SCRIPTED + validated:** `setup/migrate-v2/fix-creation-defects.ts --apply` merges `mariany` → `mariany-borges` at cutover (run once against the migrated taskflow.db). It is idempotent + transactional with a fail-loud residual scan, and rewrites every reference: `board_people` (stub DELETED on the `board-seci-taskflow` PK collision), `board_admins` (2 grants transferred — PK is `(board_id, person_id, admin_role)`), `tasks.assignee` (5), `task_history.by` (78), `archive.assignee` (1), and the token-safe JSON refs in `tasks._last_mutation`, **`tasks.notes`** (8 — caught by Codex review), `task_history.details`, `archive.task_snapshot`/`history`. Validated on a snapshot copy (all `mariany` refs → 0). (b) **Code — SHIPPED (`99010c31`, `0397a4f5`):** `register_person` now reconciles a full register into an existing role/phone-less same-name stub (`_reconcileIncompleteStub`) instead of minting a second id — guarded to exact name + incomplete + single match + candidate-not-already-present, so it only ever fills a placeholder, never merges two complete people. RESIDUAL (v2.1): the guard is exact-name (not identity), so two distinct people with a byte-identical full name where one is a transient stub could still merge; the identity-complete fix is the **canonical-people** (phone-keyed) table, which re-opens Phase-3 parity and stays deferred.
- **Status:** proposed
- **Signoff:** —
