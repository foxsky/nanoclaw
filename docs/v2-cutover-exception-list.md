...
>
> **SETD does not meet the 0-real-divergence bar.** Comparator now fixed (`4ab2c0d6`); under it SETD-INDEP = 14/40 with **4 real_divergence** (turns 3/14/26 read-instead-of-mutate; turn 13 wrong-target mutation `P8`), reproduced + verified by Codex. Numbers are now trustworthy (see the Methodology caveat in Cutover Gate Status for the residual schema-blind limitation). **Decision pending operator.** Artifacts: `/tmp/phase3-v2-results-thiago-INDEP-20260528.json`, `/tmp/phase3-thiago-flaky-{A,B}-20260528.json`.
