...
> 1. **The comparator's `task_ids` field is polluted by prose-parsed IDs — comparing it at all (strict OR lenient) is wrong for mutate turns.** `summarizeSemanticBehavior` (`scripts/phase3-support.ts:778-781`) merges IDs parsed from the confirmation *text* into `task_ids`: v1 cards carry parent-project breadcrumbs (`📁 P6`), v2 replies often lead with an `api_query` "N tarefas encontradas: …" listing. So `task_ids:[M20]->[M13,M18,M20,P6,T76,T99]` does **not** mean v2 touched 6 tasks — verified against mutation tool args, v2 mutated **only M20**. The old `allow_extra_task_ids` subset-relaxation papered over this; a naive "exact task-set parity" tightening (attempted + reverted 2026-05-29) **over-flags** 3 true matches each on SECI and SETD-projected. **Correct fix (not yet applied): for mutate turns, compare against the IDs in v2's MUTATION tool args** (`api_move/api_update_task/api_reassign/...`), not the prose-merged `task_ids` — deterministic from tool args, no model call. Until then, neither old nor "tightened" mutate-turn counts are exact.
> 2. **Single-run numbers aren't stable** (SETD: independent re-run flipped turns vs a projected baseline). Treat every single-run row as provisional; **re-run ≥2–3× and report the range.** Applies to SECI too.
>
> **Corrected provisional numbers (after the prose-pollution finding):** SECI ≈ **20/30** (not 17 from the over-flag, not 23 from the lenient subset); SETD/thiago-projected ≈ **14/40**. SETD's genuine divergences are **ACTION-level** (INDEP turns 3/14: v2 ran `read`/`api_query` and made *no* mutation call) — these survive any comparator fix and remain the real SETD blocker.
