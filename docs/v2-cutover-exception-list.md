...
> - **Turn 13 is accept-bounded — both steering levers exhausted, and no clean deterministic lever exists.** Prompt-prose (weak) AND description-redirect (0/3) both fail. A deterministic intercept can't fix it either: the agent noted **P8**, which is a *legitimate* note target — there is no unambiguous signal that P8 is wrong here. The needed disambiguation ("*Reunião sobre* Novos Sites" → prefer meeting M26 over the same-named project P8) is turn-shaped NLU, exactly the heuristic this project's rules say to walk away from when underdetermined. **Closed: accept-bounded, monitor at cutover.**
> - **Turn 26 (reminder) NOT built — accept-bounded, with a corrected source-verified capability analysis (2026-05-29).** Two layers:
>   1. **Ambiguity (the parity reason it stays open):** turn 26's replay sees **two** meetings tomorrow → v2 asking "which one?" is **defensible**. Building *any* reminder tool would NOT flip turn 26 — it would (correctly) still ask. So the replay divergence is accept-bounded on ambiguity grounds regardless of capability.
>   2. **Capability (corrected):** the earlier "it IS buildable, wrap `schedule_task` + participant resolution" claim was checked at source and is **half-right**. `schedule_task` is minute-granular and a fired scheduled task DOES land back in the board group (`src/delivery.ts:289-310` — the session's origin chat is always an allowed `send_message` destination), so a **group-post** reminder ("post in the board group 30 min before, listing participants") is genuinely deliverable and buildable. BUT **per-person DM delivery does NOT exist** in v2: `meeting_participants` (`taskflow-engine.ts`) returns no routing field per person, and `send_message` (`core.ts`) routes only by registered destination *name* — there is no `person_id`/`notification_group_jid` send path. v1's "Kipp" scheduler (empty `tool_uses`) almost certainly group-posted too. **So a per-person reminder is a real v2 capability gap; a group-post reminder is buildable but would change no replay outcome here.** Not built — shipping it during the freeze is speculative (no turn exercises it unambiguously) and a per-person variant would be a stub. **Logged as a post-cutover enhancement, not a parity blocker.**
>
> **Net SETD behavior status:** turn 14 (reschedule-by-name) FIXED + reliable; turn 3 state-drift; **turns 13 + 26 CLOSED as accept-bounded** — turn 13 after exhausting both steering levers (prose + description-redirect 0/3) and confirming no clean deterministic signal; turn 26 on genuine 2-meeting ambiguity (building a reminder tool would not flip it; per-person delivery is a separate logged capability gap). The tool-selection lever has a clear boundary: it fixes the obvious-unique-match case (turn 14), not cases where a generic tool + a known id offer an easier path (turn 13).
>
...
>
> **SETD 0-real-divergence bar:** the raw INDEP run was 14/40 with 4 real_divergence; turn 14 is now demonstrably fixed, turn 3 is state-drift, and turns 13/26 are closed accept-bounded (above). **All 4 INDEP divergences are now resolved or accept-bounded with evidence** — none is an unaddressed v2 product regression. The residual SETD gap is the *comparator/measurement* one (turn 14 still scores red because `api_reschedule_meeting` results aren't captured), not behavior. Artifacts: reschedule `/tmp/phase3-thiago-TOOL{1,2}-20260529.json`; note-steering `/tmp/phase3-thiago-NOTE-postfix-{1,2,3}.json`.
