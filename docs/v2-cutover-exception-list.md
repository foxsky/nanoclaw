...
>
> 5. **Build fix — `api_reschedule_meeting` tool — VERIFIED EFFECTIVE on its target case (2026-05-29, `35674eb0`/`e862cbdc`).** Since prompt-tuning failed, shipped a purpose tool that resolves a meeting by name (scoped to `type='meeting'`, so same-keyword simple tasks don't dilute) and reschedules the unique match. Re-ran turns 3/13/14/26 ×2 with the tool + a CLAUDE.md pointer:
...
> - **Turn 26 (reminder) NOT built** — engine reminders are day-granular; "30 min before" has no engine capability; v1 used a separate scheduler (Kipp; empty `tool_uses`); v2-asks-on-ambiguity is defensible. → **accept-bounded.**
>
