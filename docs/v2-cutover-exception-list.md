...
| Item | Finding | Done? |
|------|---------|-------|
| V1 board/user creation audit | Empirical sweep of the cutover corpus (12,737 msgs → 49 traced creation/registration episodes), cross-referenced to `taskflow.db`. **The earlier "v1 creation always worked" claim is RETIRED:** every *affirmed* request landed as at least a person row (0 omissions), but Sanunciel is a partial creation failure and the async child-board path had **6 defects**. Per-defect source verification (2026-05-30): **2 fully prevented going forward** (Hudson dup → `alreadyOnThisParent` guard, Edilson name → division-name guard), **1 boot-repaired** (Jefferson name-heal), **1 host-fixed/container-residual** (Reginaldo → FU-4), **2 that persist** → **EX-014 (Sanunciel, non-atomic skip)** + **EX-015 (Mariany, dual person_id)**. Migration repairs nothing (verbatim copy), so **3 data populations** need a one-time fix: Sanunciel orphan, Mariany dup-id, and the `board-po-setd-secti-taskflow-2` duplicate board (Hudson residual). Full map: `docs/v1-creation-empirical-map.md`. **All 6 fixes shipped; 2026-05-30 validation = source-confirmed + partial real-code replay/harness (NOT verbatim replay — the corpus has no tool layer, so scenarios are reconstructed; 3/6 are regression-harness + precondition checks). EX-015 / Jefferson / Edilson ran the real engine/boot code on forks of the cutover snapshot; EX-014 / Hudson / FU-4 via harness + snapshot precondition. Honest scope corrections (Codex 2026-05-30): EX-015 code was REMOVED (dead prevention — v2 creates no role/phone-less stub; the migration script is the operative fix). EX-014 is PARTIAL fail-loud (the createGroup/seed/wire/folder/link paths alert; parent-null / adapter-missing / link-success / confirmation-delivery-failure remain silent). Watch items — both RESOLVED 2026-05-30 (no open watch item remains): (1) Hudson dup-board recurrence is **not reachable in v2** — host delivery is serialized per session (`delivery.ts:154` `inflightDeliveries` rejects re-entry) and same-`(parent,person)` provisions originate from the same parent session, so the `alreadyOnThisParent` guard always sees the prior registration; `child_board_registrations` PK `(parent_board_id, person_id)` is a DB backstop if a second insert ever raced. A claim-first INSERT before `createGroup` would be the v2.1 hardening *only if* truly-concurrent same-session delivery is ever introduced. The existing Hudson dup is v1-legacy data, cleaned via Checklist #6. (2) `register_person` manager-auth gate (`taskflow-engine.ts:9361-9380`, manager-only): byte-identical to v1 (`/tmp/v1-snapshot/…/taskflow-engine.ts:7368`), so no parity change; and it is the correct policy — every board owner is the manager of their own board (33/33 verified), so onboarding scopes naturally down the hierarchy (each manager onboards into their board; the onboarded person becomes the manager of their own new child board). Operator decision: keep as-is, no change. Re-validated 2026-05-30 pass 2 (after EX-015 removal + /simplify): 6/6, 0 regressions — EX-015 validated via the migration script end-to-end on a snapshot fork (dry-run no-op → apply zeroes all refs → idempotent); EX-014 fail-loud confirmed through the new `alertFailed` closure.** | ✅ EX-014 + EX-015 **accepted** (operator signoff 2026-05-30); remediation = Cutover-Day Checklist #6 |
...
- **Rationale for acceptance:** Accept-and-monitor. The optimistic ack is gone and the common failure paths now fail loud to the origin chat (partial); the one historical instance (Sanunciel) is repaired at cutover by re-provisioning his child board (Checklist #6); cross-process atomicity is deferred to v2.1. Low frequency (1/33 child boards), now operator-visible on failure.
...
- **Status:** accepted
- **Signoff:** Operator (miguel.aon) accepted 2026-05-30 — accept-and-monitor; Sanunciel re-provisioned at cutover (Checklist #6); cross-process atomicity → v2.1.
...
- **Rationale for acceptance:** Accept. The migration script (Checklist #6) is the operative data-fix that merges the historical stub (re-validated incl. escaped JSON refs → 0 survivors); per-board identity is intentional design; the dead-prevention engine reconcile was removed. Canonical-people (the identity-complete prevention) is post-cutover (v2.1).
- **Mitigation / followup:** (a) **Data — SCRIPTED + validated:** `setup/migrate-v2/fix-creation-defects.ts --apply` merges `mariany` → `mariany-borges` at cutover (run once against the migrated taskflow.db). It is idempotent + transactional with a fail-loud residual scan, and rewrites every reference: `board_people` (stub DELETED on the `board-seci-taskflow` PK collision), `board_admins` (2 grants transferred — PK is `(board_id, person_id, admin_role)`), `tasks.assignee` (5), `task_history.by` (78), `archive.assignee` (1), and the JSON refs in `tasks._last_mutation`, **`tasks.notes`** (caught by an earlier Codex review), `task_history.details`, `archive.task_snapshot`/`history` — in **both** the plain JSON token `"mariany"` and the **escaped** `\"mariany\"` form (which occurs when a JSON column is serialized as a string inside another, e.g. update snapshots / archive payloads). The original merge silently left **4 escaped refs** on the live snapshot — its residual scan shared the same blind spot, so it reported a false success; the escaped-token rewrite + a **depth-agnostic** fail-loud residual scan (strip the keeper, then roll back on any surviving `mariany` at any escaping depth) were added after the Codex gpt-5.5/xhigh before-closure review (2026-05-31). Re-validated end-to-end on a snapshot fork via the real script: dry-run → `--apply` → 70 rewrites → **0 survivors**, idempotent; covered by `step-integration.test.ts`. (b) **Code — REMOVED (Simplicity-First, 2026-05-30):** the `register_person` stub-reconcile (`_reconcileIncompleteStub`, commits `99010c31`/`0397a4f5`) was reverted. Codex review established it was *dead prevention*: v2 has NO path that creates a role/phone-less `board_people` stub (`_addBoardPersonCore` always defaults `role` to `member`; `add_manager`/`add_delegate` write `board_admins`, not `board_people`), and for the actual migrated Mariany pair it was a no-op anyway. **The migration script (a) is the operative fix.** The identity-complete prevention is the **canonical-people** (phone-keyed) table (v2.1).
- **Status:** accepted
- **Signoff:** Operator (miguel.aon) accepted 2026-05-30 — migration script (Checklist #6) repairs the data; canonical-people → v2.1.
...
- **FU-1 — org-meeting display TZ bug. ✅ FIXED (commit ab2b8786, 2026-06-06):** the 4 create/draft/forward confirmation cards now route the naive `action.scheduledAt` through `fortalezaNaiveToUtcIso` (explicit -03:00 → UTC) before the formatters, so the displayed hour is correct on any host (byte-identical on Fortaleza); the engine inputs stay naive (no double-convert), the UTC-input call sites untouched. (Original finding below.) `formatTaskflowOrgMeetingDraftPrompt` / `handleTaskflowOrgMeetingCreateForwardConfirmation` format the user-facing meeting time from the un-normalized naive-local `scheduledAt` through hard-coded GMT-3 formatters. The engine stores the correct UTC value, but the displayed/forwarded hour is off by 3h under `TZ=UTC`. **Masked on this host (America/Fortaleza)** → no cutover impact for `.63`, but wrong for any other-zone deployment and unguarded by tests. Follow-up: format from the UTC-normalized value via the tz-aware helper; add a `TZ=UTC` display assertion.
- **FU-2 — sector-placement hint swallows a trailing shared-role word. ✅ FIXED (commit 71dcdb88, 2026-06-05):** added shared-role tokens (também/juntos/ambos/os dois/continuam) as positive-lookahead terminators so the hint stops at the board name. (Original finding below.) `BOARD_PERSON_PLACEMENT_RE` (with the `i` flag) captures a trailing "também" into the board hint ("SM-SETD-SECTI também"), so a trailing shared-role signal breaks board resolution. The shared-role bypass works only when the signal is separated (e.g. after a comma — covered by the new test). Follow-up: anchor the hint capture to stop before shared-role tokens.
- **FU-3 — `find_person_in_organization` LIKE-prefilter removal is INTENTIONAL + CORRECT (closed 2026-06-06 — NOT a fix; a fix would REGRESS).** Source-verified vs v1 (`main:container/agent-runner/src/taskflow-engine.ts:7138`): v1 used `LOWER(bp.name) LIKE ? ESCAPE '\'` — **name-only, no fuzzy**. v2 dropped the SQL prefilter ON PURPOSE to add matching the LIKE cannot express: **accent-insensitive** (`normalizePersonLookupText` folds accents; SQLite `LOWER` can't), **`person_id` matching** (not just name), AND a **fuzzy Levenshtein-≤1 fallback** (engine:8583-8591). So v2's matching is a strict **SUPERSET** of v1's, and **safer** (JS `.includes` treats `%`/`_` literally → no directory-enumeration vector → the removed escaping comment is genuinely moot, not a loss). Restoring a LIKE prefilter would **regress** matching (drop the accent/person_id/fuzzy hits) — the earlier "results identical" premise was false. The full-org load is the correct design and fine at ~31 boards; the fuzzy fallback's worst case (exact finds nothing) needs all rows anyway, so SQL can't prefilter it. If scale ever forces it, the only safe option is a COARSE superset prefilter that preserves the JS normalize+fuzzy passes (non-trivial), never v1's exact LIKE.
