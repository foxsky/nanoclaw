...
- **OpenAI / voice-transcription credential.** v1's `OPENAI_API_KEY` (used by `transcribe_audio`) is copied into v2 `.env` by `1a-env`, but the container reads OpenAI ONLY through the OneCLI proxy — the raw `.env` key is inert, so voice transcription returns 401 until the key is in the vault. If v2 `.env` has `OPENAI_API_KEY` AND `onecli secrets list 2>/dev/null | grep -i openai` is empty, seed it: `onecli secrets create --name OpenAI --type api_key --value <key> --host-pattern api.openai.com`, then make it visible to the owner agent (`onecli agents set-secret-mode --id <agent-id> --mode all`, or `onecli agents set-secrets`). After seeding, delete the now-inert raw key from `.env`. (Apply the same to any other container-facing key v1 used, e.g. `PARALLEL_API_KEY`.) Skip silently if v1 never set `OPENAI_API_KEY`.
