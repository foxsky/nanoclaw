...
async function processGroupMessages(chatJid: string): Promise<boolean> {
  ...
  if (!isMainGroup && group.requiresTrigger !== false) {
    // Per-group trigger pattern (TaskFlow groups use their own trigger)
    const groupTrigger = group.trigger ? buildTriggerPattern(group.trigger) : TRIGGER_PATTERN;
  ...
  try {
    const output = await runContainerAgent(
      group,
      {
        ...
        isTaskflowManaged: group.taskflowManaged || false,
        taskflowBoardId: group.taskflowManaged ? group.folder : undefined,
...
async function main(): Promise<void> {
  ...
  startIpcWatcher({
    sendMessage: (jid, text, sender) => {
      ...
      return (channel.sendMessage as (j: string, t: string, s?: string) => Promise<void>)(jid, text, sender);
    },
    registeredGroups: () => registeredGroups,
    registerGroup,
    ...
    writeGroupsSnapshot: (gf, im, ag, rj) =>
      writeGroupsSnapshot(gf, im, ag, rj),
    resolvePhoneJid: async (phone: string) => {
      for (const ch of channels) {
        if ('resolvePhoneJid' in ch && typeof (ch as any).resolvePhoneJid === 'function') {
          return (ch as any).resolvePhoneJid(phone);
        }
      }
