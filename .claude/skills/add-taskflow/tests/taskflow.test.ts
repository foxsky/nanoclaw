...
describe('taskflow skill package', () => {
  ...
  it('CLAUDE.md.template documents close approval policy and override commands', () => {
    const content = fs.readFileSync(
      path.join(skillDir, 'templates', 'CLAUDE.md.template'),
      'utf-8',
    );

    expect(content).toContain('If a task has close approval enabled');
    expect(content).toContain('exigir aprovacao para concluir TXXX');
    expect(content).toContain("requires_close_approval: true");
    expect(content).toContain('permitir concluir TXXX sem aprovacao');
