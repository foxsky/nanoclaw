...
> **Coordinated rename `api_add_task_comment` → `api_task_add_comment` (`9650b5af` nanoclaw + `1467655` tf-mcontrol, 2026-05-16).** Convention-match with the sibling `api_task_<verb>_<noun>` family (`api_task_add_note`/`edit_note`/`remove_note`) — resolves the naming debt tf-mcontrol logged in `cd2a664` (the original name was a timing-collision artifact: `01ca5932` was in flight when their rename banner landed, so they pragmatically wired the shipped name). Surgical scope: MCP tool name string + exported const (`apiAddTaskCommentTool`→`apiTaskAddCommentTool`) + `FASTAPI_ALLOWLIST` + test/entry assertions. **The internal engine method `engine.apiAddTaskComment` is intentionally NOT renamed** — not part of any cross-repo contract or the logged debt (Rule 3: don't refactor unrequested adjacent code). tf-mcontrol flipped its `call_mcp_mutation` wire + `real_mcp_client` tool-presence assertion in lockstep. Both pushed + in sync. nanoclaw: TDD RED→GREEN, byte-oracle 13/13, agent-runner 959/0, `tsc` 0. tf-mcontrol: 5 fake-mcp comment tests pass. **Activation pending:** the `@requires_real_mcp` golden re-verifies only after the `.61` dist/bun subprocess is rebuilt from `skill/taskflow-v2` (handoff BLOCKER A interim convention; same wire name both sides ⇒ stays green once rebuilt).

> **Codex review #2 (gpt-5.5/xhigh) + 0h-v2 DECIDED (2026-05-16).** 1 real bug fixed `8bae76c3`: `engine.apiAddTaskComment` assignee `(person_id=? OR name=?)` lookup was nondeterministic (no ORDER BY; `board_people.name` not unique / can collide a person_id) → wrong person notified; now `ORDER BY (person_id=?) DESC, person_id ASC LIMIT 1`. TDD RED→GREEN; tsc 0; byte-oracle 13/13; agent-runner 962/0. Codex#2: deleteBoard-detach + normalize-reorder CORRECT; the no-creator-fallback is the owner's intentional engine-canonical choice. **0h-v2 = Option A locked** (Banner #9); memo `cb0bc598` upgraded to the build spec with Codex#2's 3 required constraints (path-aware+race-safe enqueue via the `writeOutboundDirect` pattern; fail-closed routing; confirmed service-session drainability). tf-mcontrol closed the 7-golden 401 their side (`02fb72f` JWT_SECRET pin, 10/10 green). Next nanoclaw unit: implement Option A.

> **Codex review (gpt-5.5/xhigh) — 3 source-verified bugs FIXED `cb257a3a` (2026-05-16).** All confirmed at source + TDD RED→GREEN. **#5 BLOCKER:** `engine.deleteBoard` now detaches child boards (`UPDATE boards SET parent_board_id=NULL WHERE parent_board_id=?`, column-guarded) before `DELETE FROM boards` — was an FK throw for a board that is another board's hierarchy parent (canonical `boards.parent_board_id REFERENCES boards(id)`; reverses the earlier deliberate "self-FK out of scope" punt — that was unsafe). **#1 IMPORTANT:** `engine.apiAddTaskComment` resolves the assignee by `person_id OR name` + notifies/skips on the resolved person_id — `tasks.assignee` is NOT reliably a person_id (`api_update_simple_task` + legacy v1 store the display *name*), so the comment-assignee notification was silently dead for name-keyed tasks. **#3 IMPORTANT:** `api_task_add_comment` now `normalizeAgentIds` *before* validating — was rejecting the in-container `NANOCLAW_TASKFLOW_BOARD_ID` env-injection path. #4 CORRECT (no change). #2 (8-golden triage) confirmed tf-mcontrol-test-side: the 7 `_create_board` 401s are a JWT-secret mismatch (harness hardcodes `"dev-secret-change-me"` vs app `JWT_SECRET`); relayed to the tf-mcontrol handoff. container tsc 0; byte-oracle 13/13; agent-runner 960/0. Pushed nanoclaw `cb257a3a`, tf handoff `90fbcb9`.

> **REGRESSION FIX `cb06ab2a` (2026-05-16) — `api_task_add_comment` task_id mangling, caught by the `.61` activation.** The `.61` activation (rename) succeeded at infra level but the real-MCP golden surfaced a latent bug shipped in `01ca5932`: an unconditional `args.task_id.toUpperCase()`. The FastAPI subprocess runs `setVerbatimIds(true)` and passes already-resolved ids, so `task-simple` → `TASK-SIMPLE` → engine miss → `404 "Task not found: TASK-SIMPLE"`. ("task_id IS uppercased" was a misread of the handoff — it meant the **verbatim-aware** `normalizeAgentIds` path the sibling note tools use, not a blanket uppercase.) Fix: call `normalizeAgentIds(args)` exactly like `api_task_add_note` — a no-op in the FastAPI subprocess (board_id + task_id verbatim), board-inject + case-fold for the in-container WhatsApp agent. TDD RED→GREEN (verbatim-mode lowercase-id test; corrected an over-specified board_id-verbatim test to assert under the real `setVerbatimIds(true)` subprocess condition). container `tsc` 0; byte-oracle 13/13; agent-runner 960/0. **tf-mcontrol coordination:** their uncommitted `404` add_task_comment golden was re-baselined *down to this bug* (masking it); with `cb06ab2a` the path returns `201` again — that golden is reverted to the committed `201` baseline (re-baseline against fixed live MCP if desired). **`.61` re-activation required** (it was activated on the pre-fix code).

> **3rd defect FIXED + golden GREEN — `api_task_add_comment` response-contract (`ab206772` nanoclaw + `3198976` tf-mcontrol, 2026-05-16).** Root cause reproduced at source: `real_mcp_client` runs the bun MCP against `db_path` (conftest `_base_schema()`), whose `board_people` lacked the **canonical** `notification_group_jid`; for the assignee'd `task-simple` the engine's notification SELECT threw `no such column`; the handler had **no try/catch** (every sibling FastAPI tool wraps engine calls), so it escaped as JSON-RPC `-32603` → FastAPI `parse_mcp_mutation_result` (needs top-level boolean `success`) → opaque **503**. Two-part coordinated fix: nanoclaw `ab206772` adds the `try/catch → {success:false,error_code:'internal_error'}` wrapper (Rule 11, fail-loud; TDD RED→GREEN; tsc 0 / byte-oracle 13/13 / agent-runner 963/0); tf-mcontrol `3198976` adds the canonical `notification_group_jid` to conftest `_base_schema()` board_people (the established `0e72bf9` board_admins parity pattern — this is what makes it 201). **Verified: `test_golden_add_task_comment` PASSES (201) on the real bun MCP.** ⚠ The broader golden suite is **8 failed / 2 passed** — baseline before this change was 9/1, so this is **+1, zero regressions**; the other 8 (delete_board, add/remove/update_board_person, add/remove_holiday, …) are **pre-existing tf-mcontrol-side golden failures**, a separate triage (tf-mcontrol's prior "Phase 1 COMPLETE / 10-golden" status no longer holds on `.61`). `.61` live-service re-activation still pending (golden uses its own spawned subprocess, so the green is faithful; real FastAPI traffic on `.61` won't serve the fixes until uvicorn restart).
