...
- `d4b58ba0` + `c2dbf443`: #419 live-adapter fix — `normalizeAgentIds`' actor bind, the comment-author bind, and the deterministic poll-loop `senderName()` now resolve a JID-shaped authenticated sender via `board_people.phone` (shared `actor-person-resolution.ts`), binding the canonical **person_id** (not the name — name is not unique, re-resolution could escalate); phone matching is restricted to `@s.whatsapp.net`. The WhatsApp adapter restores V1's participant LID→phone translation (`resolveParticipantSender`). Your verbatim/FastAPI paths are untouched (bind still skipped under `getVerbatimIds()`).
