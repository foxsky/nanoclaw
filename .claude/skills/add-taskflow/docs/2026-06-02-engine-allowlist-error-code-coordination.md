...
7. **Pre-existing allowlisted tools' arg-shape `err()`** (out of §6 scope). The tools allowlisted before §6 — `api_create_simple_task`, `api_delete_simple_task`, the note tools — still emit raw `err()` for arg-shape rejections (e.g. `api_create_simple_task` ~`814-841` → **503** on a malformed dashboard call). §6 only hardened the 8 newly-exposed tools; bringing these to `validationError()` is a trivial follow-up (the helper is already imported) but was left untouched (Rule 3 — not in this diff). Flagged by the falsification critic (`w3t40tjo1`).
...
- **The guard blocks the 3 ORG-ENUMERATION modes; TWO further board-view mechanisms surface ancestor data and are NOT sealed (OPEN DECISION).** The arg-level guard denies the 3 modes that scan *beyond* the board's own view (`find_person_in_organization`/`find_task_in_organization`/`board_directory`, all via `orgScopeOrNull()` across the whole subtree). But the engine intentionally extends a board's OWN view past `board_id = this.boardId` by two more mechanisms — the same data the in-container WhatsApp agent for this board already sees — and the arg-level gate can't decide them (cross-board-ness depends on resolved rows, not args):
  1. **`child_exec` delegation (~25 list modes).** `visibleTaskScope()` (engine ~`1116`) adds `OR (child_exec_board_id = this.boardId AND child_exec_enabled = 1)`, and `getLinkedTasks()` (~`2422`) returns delegated parent-board task rows. Modes: `board`/`inbox`/`overdue`/`due_today`/`due_tomorrow`/`due_this_week`/`next_7_days`/`search`/`urgent`/`high_priority`/`by_label`/`statistics`/`summary`/`meetings`/`agenda`/`my_tasks`/`person_tasks`/… — they surface tasks a PARENT board delegated to this board for execution. (Found by the falsification review `w3t40tjo1`; the earlier "3 subtree + 8 lineage, no ninth" framing missed this third mechanism.)
  2. **`getVisibleTask()` participant reads (8 modes).** `task_details`, `task_history`, and the 6 meeting reads (`meeting_agenda`/`meeting_minutes`/`meeting_participants`/`meeting_open_items`/`meeting_history`/`meeting_minutes_at`) resolve via `getVisibleTask()` (~`1891`), which returns a PARENT/ancestor-board meeting when this board is a **participant**.

  Both return ANCESTOR data this board legitimately **executes/participates in** (not org-wide enumeration), mirroring the in-container agent — which is why they are out of the denylist. **Decision deferred to the dashboard owner:** (a) accept this delegated-in + participant view on the dashboard (mirrors the WhatsApp agent), or (b) seal to a STRICTER board-only view via a board-strict engine flag (collapse `visibleTaskScope` to `board_id = ?` and use `getTask()` not `getVisibleTask()` on the FastAPI path) — a deeper cross-consumer change. No dashboard route calls these `api_query` modes yet (forward-prep exposure).
