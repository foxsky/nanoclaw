...
- **The guard covers only the SUBTREE-SCAN modes — 8 LINEAGE-based reads are NOT sealed (OPEN DECISION).** The arg-level guard denies the 3 modes that are cross-board *by name* (`find_person_in_organization`/`find_task_in_organization`/`board_directory`, all via `orgScopeOrNull()`). But `task_details`, `task_history`, and the 6 meeting reads (`meeting_agenda`/`meeting_minutes`/`meeting_participants`/`meeting_open_items`/`meeting_history`/`meeting_minutes_at`) resolve their task via `getVisibleTask()` (engine ~`1891`), which walks the board **lineage** and returns a PARENT/ancestor-board meeting when the requesting board is a **participant** (read-only, participant-gated, ancestor-only — pre-existing engine behaviour, newly *exposed* on FastAPI by allowlisting `api_query`). The arg-level gate can't decide this (the cross-board-ness depends on the *resolved* task's board, not the args). **Decision deferred to the dashboard owner:** (a) accept participant-gated ancestor-meeting visibility on the dashboard (it mirrors the in-container agent, which a child board's agent already has), or (b) seal it to strict board-locality via a board-strict engine flag that uses `getTask()` instead of `getVisibleTask()` on the FastAPI path (a deeper cross-consumer change). No dashboard route calls these modes yet, so nothing leaks today — this is forward-prep exposure. Found by the build-time adversarial review (`wh452jbvu`); the original §6/§8 framing of "exactly 3 cross-board modes" was incomplete.
