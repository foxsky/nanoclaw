/**
 * MIRROR of the host RunnerState computation at src/modules/taskflow/runner-state.ts — keep the two
 * in sync. Host and container share no modules (separate runtimes: better-sqlite3 vs bun:sqlite), so
 * this is a deliberate copy; only the DB driver type differs. The SQL, the cron-window logic and the
 * local-date reasoning are identical so a warm container gates exactly as the host sweep would.
 *
 * Reads the shared taskflow.db (tasks + task_history, board_id-scoped) and the session's inbound.db
 * (member messages = kind='chat'; runner rows are kind='task'). The "since last run" window is the
 * runner's own previous cron occurrence. All day/date reasoning is in the board's local timezone
 * (the crons are stored local), so weekend/Monday/due-today land on the right calendar day.
 */
...
function count(db: Database, sql: string, ...params: unknown[]): number {
  const row = db.prepare(sql).get(...(params as never[])) as { n: number } | null;
  return row?.n ?? 0;
}
