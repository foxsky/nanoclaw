...
Use available_groups.json to find the JID for a group. The folder name should be lowercase with hyphens (e.g., "family-chat").`,
  {
    ...
    requiresTrigger: z
      ...
      .optional()
      .describe(
        'Whether messages must start with the trigger word. Default: false for TaskFlow groups, true otherwise.',
      ),
  ...
  async (args) => {
    ...
    const data = {
      ...
      requiresTrigger: args.requiresTrigger ?? (args.taskflow_managed ? false : true),
...
if (process.env.NANOCLAW_IS_TASKFLOW_MANAGED === '1') {
  ...
  if (boardId) {
    ...
    server.tool(
      ...
      'Query the TaskFlow board. Returns structured data for board views, task details, search, statistics, etc.',
      {
        query: z.enum(['board', 'inbox', 'review', 'in_progress', 'next_action', 'waiting',
          ...
          'find_person_in_organization']).describe('Query type'),
    ...
    server.tool(
      ...
      {
        action: z.enum(['register_person', 'remove_person', 'add_manager', 'add_delegate', 'remove_admin', 'set_wip_limit', 'cancel_task', 'restore_task', 'process_inbox', 'manage_holidays', 'process_minutes', 'process_minutes_decision', 'accept_external_invite', 'reparent_task', 'detach_task', 'merge_project', 'handle_subtask_approval']).describe('Admin action'),
        ...
        decision: z.enum(['create_task', 'create_inbox', 'approve', 'reject']).optional().describe('Decision for process_minutes_decision (create_task/create_inbox) or handle_subtask_approval (approve/reject)'),
        request_id: z.string().optional().describe('Request ID for handle_subtask_approval (e.g., "req-1744200000-a3f2")'),
        reason: z.string().optional().describe('Reason for handle_subtask_approval when decision=reject'),
        create: z.object({
          type: z.string(),
          title: z.string(),
