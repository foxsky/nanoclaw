...
ALTER TABLE boards ADD COLUMN owner_person_id TEXT;
  ...
  describe('admin', () => {
    ...
    it('EX-015: reconciles a full register into an incomplete same-name stub (no second person_id)', () => {
      // A delegate/grant left a role+phone-less stub whose id came from a shorter
      // name input but whose name field is the full "Mariany Borges".
      db.prepare(
        `INSERT INTO board_people (board_id, person_id, name, phone, role) VALUES (?, 'mariany', 'Mariany Borges', NULL, NULL)`,
      ).run(BOARD_ID);
      ...
      const r = engine.admin({
        ...
        group_name: 'SEMCASPI - TaskFlow',
        group_folder: 'semcaspi-taskflow',
      });

      expect(r.success).toBe(true);
      // reused the stub id, did NOT mint 'mariany-borges'
      expect(r.person_id).toBe('mariany');
      expect(r.auto_provision_request?.person_id).toBe('mariany');
      const rows = db
        .prepare(`SELECT person_id, phone, role FROM board_people WHERE board_id=? AND lower(name)='mariany borges'`)
        .all(BOARD_ID) as Array<{ person_id: string; phone: string; role: string }>;
      expect(rows).toHaveLength(1);
      expect(rows[0].person_id).toBe('mariany');
      expect(rows[0].phone).toBe('5586981352365');
      expect(rows[0].role).toBe('Analista de Inovação');
      expect(db.prepare(`SELECT 1 FROM board_people WHERE board_id=? AND person_id='mariany-borges'`).get(BOARD_ID)).toBeNull();
    });

    it('EX-015: does NOT reconcile into a COMPLETE same-name person (only incomplete stubs)', () => {
      db.prepare(
        `INSERT INTO board_people (board_id, person_id, name, phone, role) VALUES (?, 'other-id', 'Mariany Borges', '5586000000000', 'Gerente')`,
      ).run(BOARD_ID);
      ...
      expect(r.success).toBe(true);
      expect(r.person_id).toBe('mariany-borges'); // minted fresh, did NOT reuse other-id
      const other = db.prepare(`SELECT role, phone FROM board_people WHERE board_id=? AND person_id='other-id'`).get(BOARD_ID) as {
        role: string;
        phone: string;
      };
    ...
    it('EX-015: does NOT reconcile when the same-name stub is ambiguous (>1)', () => {
      ...
      expect(r.success).toBe(true);
      expect(r.person_id).toBe('ana-lima'); // fresh slug; ambiguous stubs untouched
    });
