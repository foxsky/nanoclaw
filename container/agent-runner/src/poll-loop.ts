...
export interface PollLoopConfig {
  ...
  /**
   * Compatibility shim for the install-overlay split (ADR 0006). The shared
   * container entry `index.ts` is NOT overwritten by `/add-taskflow`, but the
   * TaskFlow poll-loop overlay (which DOES replace this file) consumes these.
   * Accepted-and-ignored here so the pristine-core container typechecks against
   * the unmodified entry; the overlay gives them meaning. Trivially upstreamable.
   */
