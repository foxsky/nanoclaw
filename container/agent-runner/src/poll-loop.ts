...
function handleTaskflowMissingExactIdNote(
  ...
  if (orgRows.length > 0) {
    ...
    if (noteResult.success) {
      // EX-019: apiAddNote now builds the owner/parent notification (V1 parity),
      // so deliver it like the MCP api_task_add_note tool does. The reassign half
      // below dispatches its own.
...
export function handleTaskflowProjectNoteUpdate(
  ...
  // EX-019: apiAddNote now builds the owner/parent notification (V1 parity) —
  // deliver it like the MCP api_task_add_note tool does.
