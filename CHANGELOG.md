...
## 2026-04-30 — Cross-board forward rule: engine-first ordering + forbidden-phrase guidance

Closes the gap surfaced by Kipp on the 2026-04-27 P11 case (msg `3EB04ABD3417EAA72CA13F`, board-asse-seci-taskflow). Lucas Batista asked `"adicionar tarefa na P11: Acompanhamento de Processo do Bruno Crispim de João Pessoa"`; the bot recognized P11 belonged to the parent SECI board (visible via `[seci]` prefix in its task views) and refused with `"O projeto P11 — Operação da SECTI pertence ao quadro da SECI (nível acima). Não consigo adicionar etapas a ele diretamente por aqui. Para adicionar a etapa..., você precisará fazer isso pelo quadro da SECI."` — never invoked `send_message` to forward. The Phase-1a `Cross-board add_subtask forward` rule (shipped 2026-04-27) was gated on `taskflow_update` returning `task not found`; preemptive recognition bypassed the rule entirely.

**Rule strengthening** (`scripts/migrate-claude-md-cross-board-forward.mjs` + `.claude/skills/add-taskflow/templates/CLAUDE.md.template`). The trigger paragraph now says the agent must always call `taskflow_update({ task_id: 'PXXX', updates: { add_subtask: ... } })` first — even when it can name the parent board from a `[seci] P11`-prefixed view. The engine remains the source of truth for what is writable from this board, so `cross_board_subtask_mode` (`open` / `blocked` / `approval`) for delegated tasks still resolves through the engine, not through manual proxy-forwarding. Forwarding is now positioned exclusively as the `task not found` engine fallback. New "Forbidden refusal patterns" block lists the literal Portuguese fragments observed in the bug (`pertence ao quadro`, `precisará fazer pelo quadro`, `faça por lá`) as fragments-only — no full sentence quotes — to reduce LLM copy-bias.

**SQL fix** (same files). The "look up the parent" verification query referenced `b_parent.name`, which doesn't exist on the `boards` table (schema is `id, group_jid, group_folder, board_role, hierarchy_level, max_depth, parent_board_id, short_code, owner_person_id`). Replaced with `COALESCE(NULLIF(b_parent.short_code, ''), b_parent.group_folder, b_parent.id)`. Added `t.type = 'project'` filter so the join doesn't match same-ID regular tasks on the parent board (per-board `(board_id, id)` PK means `P11` legitimately exists multiple times across the fleet).

**Codex pre-merge review caught two HIGH risks in v1 of this fix.** Initial draft introduced a "preemptive forward" trigger that would have skipped the engine entirely for delegated parent tasks — bypassing `cross_board_subtask_mode='blocked'` and `'approval'` branches and turning every `[seci]`-prefixed reference into a manual forward, even when the engine governs the path. SQL walk-up was also flagged as an insufficient false-forward gate (per-board ID collisions). v2 restored engine-first ordering and added the `t.type = 'project'` filter; both HIGH findings resolved in the followup review.

**Tests** (`.claude/skills/add-taskflow/tests/migrate-cross-board-forward.test.ts`). 4 new tests, 9 total. Asserts: (1) the rule mentions recognition AND bans preemptive refusal; (2) the rule forbids the literal Portuguese fragments; (3) engine-first ordering — rule says `try the tool first` and references `taskflow_update`; (4) SQL doesn't reference `b_parent.name`, uses `group_folder`/`short_code`, and filters `t.type = 'project'`. Full taskflow skill suite green at 383/383; typecheck clean. 11 generator-managed `groups/*/CLAUDE.md` regenerated.
...
**Same-failure dedup for merged audit deviations** (`container/agent-runner/src/semantic-audit.ts` + `auditor-script.sh`). New exported `dedupeDeviations` collapses LLM-framing duplicates produced by the response and mutation passes (deepseek-v4-pro emitted 1.86× records per unique bot reply on 2026-04-29; Sonnet 1.33× per task+field). Response key: `(boardId, responseMessageId)`. Mutation key: `(boardId, taskId, fieldKind, at)` — `at` is the source `task_history.at` so two genuinely different same-day mutations on the same field are kept. On collision, picks the higher-confidence record (`high > med > low`), then longer deviation prose, then first-emitted on tie. Rank table aligned to the actual `Confidence` union (`high|med|low`) — Codex caught a `'medium'` typo that would have ranked real `'med'` records as 0. Wired into `auditor-script.sh` between merge and the dryrun/enabled mode branches so both consume the deduped set. Known limitation tracked separately: same-millisecond concurrent writes to the same task+field could still collide on the `at`-based key; carrying `task_history.id` through is the followup fix.

**`sourceMutationId` as the canonical mutation dedup anchor** (`container/agent-runner/src/semantic-audit.ts`). Closes the IMPORTANT Codex flagged on the `at`-based mutation key earlier today: two genuinely distinct same-millisecond writes to the same task+field would still collide under that key. New `SemanticDeviation.sourceMutationId` carries the `task_history.id` PK (auto-increment, schema at `src/taskflow-db.ts:107`); `dedupeDeviations` keys mutations on `(boardId, sourceMutationId)` when present, falls back to `(boardId, taskId, fieldKind, at)` for back-compat with NDJSON records that predate this field. Response-pass branch unchanged.

**Multi-action-turn rule in TaskFlow CLAUDE.md template** (`.claude/skills/add-taskflow/templates/CLAUDE.md.template`). New "## Multi-Action Turns" section addresses the bug catalog at `docs/incidents/2026-04-29-multi-action-ignored.md`: 14/14 unique response-pass deviations on 2026-04-29 fit the pattern of "user message has 2+ write actions, bot processes only the first." The rule tells the agent to identify every action instance (including repeated uses of the same verb — two `adicionar nota` requests are TWO actions), execute each via its own `taskflow_update` call in sequence, and produce one bullet-list reply at the end. Includes anti-pattern + correct-pattern examples using the canonical `taskflow_update({ task_id, updates, sender_name })` shape — Codex caught my first draft using the engine's internal `update_task` interface name instead of the actual MCP tool name. Rendered into all 11 TaskFlow boards via `scripts/generate-claude-md.mjs` (deploy regen step).
