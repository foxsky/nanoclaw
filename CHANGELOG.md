...
- Closed the remaining v2 Taskflow cutover gates: added a cutover rollback/canary runbook, recorded operator signoff for v1-bug-corrected exceptions, verified prompt regeneration idempotency across 36 boards with no legacy sqlite/tool-name leaks, and documented the isolated prod-snapshot migration dry-run plus restart/dedup recovery test evidence.
- Wired v1→v2 prompt migration into `setup/migrate-v2/groups.ts` so cutover writes regenerated `CLAUDE.local.md` files instead of raw v1 board prompts, and removed stale v1 `schedule_task` schema examples (`target_group_jid`, `schedule_type`, `schedule_value`) from regenerated prompts.
- Replaced the last raw-sqlite prompt fallback for cross-board subtask mode with first-class `api_admin({ action: "set_cross_board_subtask_mode" })` support, including engine persistence/history, MCP schema validation, template regeneration, and migration-script rewrites for existing board prompts.
- Fixed SEC Phase 3 duplicate-create parity: `api_create_task` now returns a v1-style reuse/separate-task question when an active same-assignee task has a near-identical title, and the semantic comparator treats those duplicate warnings as ask behavior rather than successful mutations.
- Fixed SEC Phase 3 meeting external-participant follow-up parity: replies like "Edgar é participante externo: +55..." after an unresolved meeting participant prompt now update the just-created meeting through `api_update_task`/`add_external_participant`, preserving the meeting participant registry and task history instead of sending a standalone OTP/invite.
- Fixed Laizys/SEAF Phase 3 clarification parity: child-board creation requests now ask for responsible name, phone, and role before provisioning; the subsequent contact-card turn validates through context-chain replay as an `api_admin` board mutation; incomplete note requests with a task id now ask for the note text instead of inventing one; and the semantic comparator recognizes Portuguese "preciso saber / pode me informar" clarification replies as asks.
- Optimized SETD/Thiago Phase 3 parity: exact-ID next-action updates now mutate through the native Taskflow engine instead of being misread as outbound-message requests, STRANS-style search now ranks task notes and waiting text, organization/role/person directory questions answer with one `api_query`, and SETD sector-person placement uses direct `api_add_board_person`/`api_remove_board_person` calls without model-driven Monitor loops.
- Hardened Laizys/SEAF Phase 3 parity: child-board provisioning now derives `group_folder` from the division `group_name` instead of falling back to person-based board names, completed-task reassignment failures use the v1-style "create a new task?" question, and the semantic comparator treats scheduling/system rows as first-class mutations without polluting user-facing intent.
...
- Optimized ASSE Phase 3 turn 26 (`minhas tarefas`): `my_tasks`/`person_tasks` now return a formatted v1-style person task summary, and the Taskflow poll loop handles self task-list requests deterministically with one `api_query` instead of letting the model fan out into repeated queries and Monitor/TaskOutput parsing.
