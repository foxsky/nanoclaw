...
Still to do manually on production: add `TimeoutStopSec=120` to `/home/nanoclaw/.config/systemd/user/nanoclaw.service` so the 20s dispatcher drain plus queue.shutdown plus channel.disconnect fits comfortably inside the timeout. Default 90s was mathematically enough but empirically produced three `final-sigterm timed out. Killing` events in one morning, meaning real shutdown paths took longer than their accounting suggests.
