...
Closes the gap surfaced by Kipp on the 2026-04-27 P11 case (msg `3EB04ABD3417EAA72CA13F`, board-asse-seci-taskflow). Lucas Batista asked to add a subtask on `P11`; the bot recognized P11 belonged to the parent SECI board (visible via `[seci]` prefix in its views) and refused with `"pertence ao quadro da SECI ... você precisará fazer isso pelo quadro da SECI"` instead of forwarding. The Phase-1a `Cross-board add_subtask forward` rule (shipped 2026-04-27) was gated on `taskflow_update` returning `task not found`; preemptive recognition bypassed the rule entirely.

**Rule strengthening** (`scripts/migrate-claude-md-cross-board-forward.mjs` + `.claude/skills/add-taskflow/templates/CLAUDE.md.template`). Trigger now mandates calling `taskflow_update` first, even with preemptive cross-board recognition — the engine remains the source of truth for `cross_board_subtask_mode` (`open` / `blocked` / `approval`) on delegated tasks. Forwarding is the `task not found` fallback only. New "Forbidden refusal patterns" block lists the literal Portuguese fragments from the bug (`pertence ao quadro`, `precisará fazer pelo quadro`, `faça por lá`) as fragments to ban, no full-sentence quotes (reduces LLM copy-bias).

**SQL fix** (same files). The parent-walk query referenced `b_parent.name`, which doesn't exist on the `boards` table. Replaced with `COALESCE(b_parent.short_code, b_parent.group_folder, b_parent.id)`. Added `t.type = 'project'` filter so the join doesn't match same-ID regular tasks on the parent (per-board `(board_id, id)` PK means `P11` legitimately exists across the fleet).

**Tests** (`.claude/skills/add-taskflow/tests/migrate-cross-board-forward.test.ts`). 4 new tests, 9 total: rule must mention recognition + ban preemptive refusal; must forbid the literal Portuguese fragments; must enforce engine-first (`try the tool first` + `taskflow_update`); SQL must not reference `b_parent.name`, must use `group_folder`/`short_code`, must filter `t.type = 'project'`. The four content-assertion tests share one fixture+migration via `beforeAll` (saves ~700ms). Full taskflow suite 383/383 green; typecheck clean. 11 generator-managed `groups/*/CLAUDE.md` regenerated.

**Deploy.** The standard `migrate-claude-md-cross-board-forward.mjs` script is idempotent and skips boards that already have the rule marker — so re-running it after today's wording change does nothing for boards that received v1 yesterday. New one-shot `scripts/upgrade-cross-board-forward-v1-to-v2.mjs` finds the v1 span (anchored on its last line `fall back to the existing "task not found" refusal.` rather than the following `**Handling subtask-approval**` section, since 19 older provisioned boards predate that section) and replaces it with v2. `V2_SENTINEL` (the new "Forbidden refusal patterns" string) makes the script idempotent and safe to re-run. Prod sweep result: **31/31 TaskFlow boards on v2** (12 via `deploy.sh` rsync of regenerated locals, 19 via the upgrade script for prod-only boards provisioned through `provision-shared.ts`). Service restarted, WhatsApp reconnected.
