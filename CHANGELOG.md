...
- Hardened Phase 3 metadata matching for regenerated production corpora: annotations can now bind to stable source identifiers (`source_turn_id`, `source_jsonl`, timestamp+text) and index-only metadata is ignored for source-keyed generated turns, preventing stale v1-bug/state-drift notes from following a reused numeric turn index.
