...
```
Subtasks have `parent_task_id` set. Use SQL: `SELECT * FROM tasks WHERE (board_id = 'board-ux-setd-secti-taskflow' OR (child_exec_board_id = 'board-ux-setd-secti-taskflow' AND child_exec_enabled = 1)) AND assignee = ? AND parent_task_id IS NOT NULL AND column != 'done'`
