...
export const DIGEST_PROMPT =
  '[TF-DIGEST] You are generating a SHORT evening resumo for this task group. The host only fires this digest on days the board was active, so keep it brief — a resumo, NOT the full board breakdown. Query the board from /workspace/taskflow/taskflow.db using the SQLite MCP tools — SELECT from tasks for your board_id. If no tasks exist on your board AND no parent-board tasks are assigned to your people, do NOT send any message — exit silently. Otherwise send a brief resumo (a few lines): the headline counts (overdue / due in next 48h / completed today) and the single most important follow-up for tomorrow with its task ID. Do NOT enumerate every task or every column. Send the resumo to this group via send_message. Note: send_message sends to this group only — individual DMs are not supported.' +
  PARENT_BOARD_HINT;
