...
/**
 * The #398 notification-relay-prose fix in isolation: exact stale-string → host-
 * delivers-text replacements (each idempotency-guarded), and NOTHING ELSE — no
 * blanket tool renames or `target_chat_jid`→`to` rewrites. Safe to run over a
 * whole deployed `CLAUDE.local.md` that mixes rendered instructions with agent
 * free-text memory (the #404 re-patch runner), because the matched strings are
 * the long, specific stale-relay sentences that won't occur in arbitrary memory.
 */
export function patchRelayProse(input: string): { output: string; changed: boolean } {
  let output = input;
...
export function migrateBoardClaudeMd(input: string): MigrationResult {
  ...
  // The #398 relay-prose fix — extracted (patchRelayProse) so the #404 re-patch
  // runner can apply ONLY this (safe over agent memory), without the blanket
  // tool renames above.
  output = patchRelayProse(output).output;
