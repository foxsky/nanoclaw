...
export function migrateBoardClaudeMd(input: string): MigrationResult {
  ...
  output = output.replace(
    ...
    '## Notification Dispatch\n\n' +
      'The v2 host delivers cross-chat notifications routed by JID — anything the ' +
      'engine targets via `notification_group_jid`, `target_chat_jid`, or ' +
      '`parent_group_jid` (direct/DM and parent rollups). **Do NOT relay those** — the ' +
      'host already delivers them; relaying causes a duplicate send.\n\n' +
      '**Still relay these (the host does not deliver them yet):** entries routed by ' +
      '`destination_name` (a `destination_message`) via ' +
      '`send_message({ to: destination_name, text: message })`, and any ' +
      '`pending_approval` envelope via `send_message({ to: pending_approval.destination_name, text: pending_approval.message })`. ' +
      'Do NOT modify the notification text.\n\n' +
      'Your normal assistant reply still covers the current chat; never create a same-group duplicate.\n\n',
  );

  // #398: neutralize the OTHER relay sites a v1 board carries (the section
  // regex above only reaches "## Notification Dispatch"). On the fixed v2
  // template these anchors no longer match (the template edits already
  // applied the fix), so each is a guarded no-op there and only fires on a
  // real v1 board CLAUDE.local.md during migration. Without these, an
  // existing board still double-sends JID/parent-routed notifications the
  // v2 host now delivers.
  if (!output.includes('**Engine notifications are delivered by the host')) {
    output = output.replace(
      ...
      '**Engine notifications are delivered by the host, not relayed by you.** `notifications` and `parent_notification` come preformatted from the engine. The host delivers every notification routed by a JID — `notification_group_jid`, `to`, or `parent_group_jid` — so do NOT relay those (duplicate send). The ONLY entries you still relay are those routed by `destination_name` (a `destination_message`), plus any `pending_approval` envelope.',
    );
  }
  if (!output.includes('relay only `destination_name` entries')) {
    output = output.replace(
      ...
      '- If `notifications` or `parent_notification` is present, see Notification Dispatch — the host delivers JID/group/parent-routed entries; relay only `destination_name` entries',
    );
  }
  if (!output.includes('Relay only `destination_name`-routed entries (see Notification Dispatch)')) {
    output = output.replace(
      ...
      'Notifications are generated by the engine. The host delivers the JID/group/parent-routed ones — do NOT relay those. Relay only `destination_name`-routed entries (see Notification Dispatch). Do NOT add separator lines or invent same-group duplicates.',
    );
  }
  // Invite-pending "Convite pendente" card carries no JID — it is an in-chat
  // honesty/forward card, NOT a cross-chat relay (the normalizer throws
  // `missing routing target` on it). Strike only the cross-chat relay
  // directive; the "do NOT say convites enviados" honesty sentence above is
  // outside this anchor and survives.
  if (!output.includes('it carries no JID and is not a cross-chat relay')) {
    output = output.replace(
      ...
      'Check the `notifications` array in the tool response — if it contains `target_kind: \'group\'` with "Convite pendente" or "Reconvite pendente", the invite was NOT sent as a DM. Deliver the honest in-chat explanation above to the user in the current group. Do NOT `send_message` this notification to another chat — it carries no JID and is not a cross-chat relay.',
    );
  }
