...
export class EmbeddingService {
  ...
  async runIndexerCycle(): Promise<void> {
    ...
    try {
      ...
      if (!response?.ok && this.fallbackOllamaHost) {
        log.info('Embedding indexer: primary failed, trying fallback', {
      ...
      if (!response?.ok) {
        log.warn('Embedding indexer: Ollama request failed', {
      ...
      if (!data.embeddings || data.embeddings.length !== pending.length) {
        log.warn('Embedding indexer: unexpected embedding count from Ollama');
        return;
      }
      ...
      log.info('Embedding indexer: batch processed', {
        count: pending.length,
      });
    } catch (err) {
      log.warn('Embedding indexer cycle failed', { err });
    } finally {
  ...
  private async fetchEmbed(
    ...
    try {
      ...
      log.warn('Embedding fetch failed', { err, host });
      return null;
    }
  }
}
